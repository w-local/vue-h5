...
function transitionExtend(orgin:any) {
  // 通过原路由对象创建一个新的对象
  ...
  // 路由位置字符串在数组中的位置
  router.indexOf = function (path:any) {
  ...
  // 添加历史路由去路由数组
  router.addRouterPath = function(path:any) {
  ...
  // 初始化，为了页面刷新能恢复路由记录等
  router.initRouterPaths = function (toPath:any) {
